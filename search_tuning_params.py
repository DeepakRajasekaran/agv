...
def run_test(kp, kd):
    # Safe cleanup targeting only the simulation nodes
    subprocess.run(["pkill", "-f", "play.launch.py"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    while rclpy.ok():
        ...
        # 30 seconds is plenty to navigate the path
...
if __name__ == "__main__":
    rclpy.init()
    candidates = [
        # Search lower kp to minimize overshoot and higher kd to damp delays
        (1.5, 0.5), (1.5, 1.0), (1.5, 1.5),
        (2.0, 0.5), (2.0, 1.0), (2.0, 1.5),
        (3.0, 0.5), (3.0, 1.0), (3.0, 1.5), (3.0, 2.0),
        (4.0, 0.5), (4.0, 1.0), (4.0, 1.5), (4.0, 2.0),
        (5.0, 0.5), (5.0, 1.0), (5.0, 1.5), (5.0, 2.0)
    ]
            
    results = []
    for kp, kd in candidates:
        # Safe cleanup before each run
        subprocess.run(["pkill", "-f", "play.launch.py"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(["pkill", "-f", "hardware_sim_node"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(["pkill", "-f", "line_follower_controller_node"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(["pkill", "-f", "nav_server_node"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
